...
class CatForm extends MchModel
{
    ...
    {
        $topicCountQuery = Topic::find()->select('count(1)')->alias('t')
            ->where(['store_id' => $this->store, 'is_delete' => 0])
            ->andWhere('t.type=id');

        $query = TopicType::find()->select(['*','goods_count' => $topicCountQuery])
