...
Page({
                    ...
                    success(res) {
                        ...
                        getApp().request({
                            ...
                            success(res) {
                                ...
                                let nowAdd = nextAdd;
                                // 如果有助力，计算当前加成
                                if (user.invite_list.length > 0) {
                                    let ratio = 0;
                                    user.invite_list.forEach(function(e) {
                                        friend.push(e.avatar_url)
                                        if (e.create_time > today) {
                                            ratio += +e.invite_ratio                                          
                                        }
                                        nowAdd = (user.ratio - ratio) / 10;
                                    })
                                }
                                let checked;
                                // 每日兑换提醒开关
    ...
    onLoad: function(object) {
        ...
        if (object.scene !== null) {
            const scene = decodeURIComponent(object.scene)
